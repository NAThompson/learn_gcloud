...
\begin{frame}[fragile]
  \frametitle{Exercise}
  Construct a load-balancing, fault tolerant, autoscaling http server. Make sure the autoscaling metric is http traffic.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{minted}{bash}
$ gcloud compute instance-groups managed create server-group \
 --zone us-central1-b --template server-template \
 --size 1 --base-instance-name server
  \end{minted}
  And the autoscaling group:
  \begin{minted}{bash}
$ gcloud compute instance-groups managed set-autoscaling server-group \
 --zone us-central1-b --cool-down-period 180 --max-num-replicas 5 \
 --min-num-replicas 1 --target-cpu-usage 0.3
  \end{minted}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Solution}
  Now configure your target pool:
\begin{minted}{bash}
$ gcloud compute target-pools create mypool --region us-central1
$ gcloud compute forwarding-rules create myforwardingrule \
   --target-pool mypool --region us-central1
$ gcloud compute http-health-checks create check80
$ gcloud compute target-pools add-health-checks mypool \
   --http-health-check check80 --region us-central1
  \end{minted}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Solution}
  The following will request the image over and over:
  \begin{minted}{bash}
    $ for i in {1..1000}; \
    do wget --no-parent --accept=jpg,jpeg,htm,html \
    --mirror  http://104.197.182.162/; \
    rm -rf 104.197.182.162/; done;
  \end{minted}
\end{frame}
